...
def main():
    ...
    with col1:
        ...
        pdf_file = st.file_uploader("📤 Upload PDF", type=["pdf"], key="file_upload")
        st.markdown('</div>', unsafe_allow_html=True)

    with col2:
        st.markdown('<div class="upload-card">', unsafe_allow_html=True)
        pdf_url = st.text_input("🌐 PDF URL", placeholder="Enter document URL...", key="url_input")
        st.markdown('</div>', unsafe_allow_html=True)
        ...
        with st.spinner("🔍 Analyzing document..."):
            ...
            with col_right:
                with st.expander("🖼️ Visual Preview", expanded=True):
                    ...
                    try:
                        images = pdf2image.convert_from_path(pdf_path, dpi=100, first_page=selected_pages[0], last_page=selected_pages[-1])
                        for img in images:
